...
import { useSalesSummary } from '@/services/dashboard';

export default function AnalyticsPage() {
  const { data: analyticsData } = useBusinessAnalytics();
  const { data: salesData } = useSalesSummary();

  const rawCampaignStats = (analyticsData as any)?.campaignStats;
  const campaignStats: any[] = rawCampaignStats?.length ? rawCampaignStats : [
  ...
  const rawPopularRewards = (analyticsData as any)?.popularRewards;
  const popularRewards: any[] = rawPopularRewards?.length ? rawPopularRewards : [
  ...
  const rawCustomerMetrics = (salesData as any)?.customerMetrics;
  const customerMetrics: any[] = rawCustomerMetrics?.length ? rawCustomerMetrics : [
