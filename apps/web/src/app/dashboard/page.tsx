...
import { useDashboardSummary, useSalesSummary, useLiveMonitoring } from '@/services/dashboard';
import { useCustomerActivity } from '@/services/customer';
...
export default function DashboardOverview() {
  ...
  const activityFeed = React.useMemo(() => {
    if (!summaryData?.recentActivity) return [];
    return summaryData.recentActivity.map((session: any) => {
      const name = session.customer ? `${session.customer.firstName || ''} ${session.customer.lastName || ''}`.trim() : 'Guest Customer';
      return {
        id: session.id,
        icon: 'target',
        text: `${name} initiated a play session`,
  ...
  const kpis: any[] = summaryData ? [
    ...
    { label: 'Total Plays', value: summaryData.totalPlays ?? 0, change: '', trend: 'neutral', icon: (
      <svg className="w-5 h-5" fill="none" stroke="currentColor" strokeWidth="1.8" viewBox="0 0 24 24">
        <path strokeLinecap="round" strokeLinejoin="round" d="M3.75 13.5l10.5-11.25L12 10.5h8.25L9.75 21.75 12 13.5H3.75z" />
      </svg>
    )},
    { label: 'Rewards Redeemed', value: summaryData.rewardsRedeemed ?? 0, change: '', trend: 'neutral', icon: (
      <svg className="w-5 h-5" fill="none" stroke="currentColor" strokeWidth="1.8" viewBox="0 0 24 24">
        <path strokeLinecap="round" strokeLinejoin="round" d="M21 11.25v8.25a1.5 1.5 0 01-1.5 1.5H5.25a1.5 1.5 0 01-1.5-1.5v-8.25M12 4.875A2.625 2.625 0 109.375 7.5H12m0-2.625V7.5m0-2.625A2.625 2.625 0 1114.625 7.5H12m0 0V21m-8.625-9.75h18c.621 0 1.125-.504 1.125-1.125v-1.5c0-.621-.504-1.125-1.125-1.125h-18c-.621 0-1.125.504-1.125 1.125v1.5c0 .621.504 1.125 1.125 1.125z" />
      </svg>
    )},
    { label: 'Customer Engagement', value: summaryData.customersEngaged ?? 0, change: '', trend: 'neutral', icon: (
      <svg className="w-5 h-5" fill="none" stroke="currentColor" strokeWidth="1.8" viewBox="0 0 24 24">
        <path strokeLinecap="round" strokeLinejoin="round" d="M15 19.128a9.38 9.38 0 002.625.372 9.337 9.337 0 004.121-.952 4.125 4.125 0 00-7.533-2.493M15 19.128v-.003c0-1.113-.285-2.16-.786-3.07M15 19.128v.106A12.318 12.318 0 018.624 21c-2.331 0-4.512-.645-6.374-1.766l-.001-.109a6.375 6.375 0 0111.964-3.07M12 6.375a3.375 3.375 0 11-6.75 0 3.375 3.375 0 016.75 0zm8.25 2.25a2.625 2.625 0 11-5.25 0 2.625 2.625 0 015.25 0z" />
      </svg>
    )},
    { label: 'Redemption Rate', value: `${summaryData.redemptionRate ?? 0}%`, subtext: 'Redeemed', change: '', trend: 'neutral', icon: (
      <svg className="w-5 h-5" fill="none" stroke="currentColor" strokeWidth="1.8" viewBox="0 0 24 24">
        <path strokeLinecap="round" strokeLinejoin="round" d="M9 12.75L11.25 15 15 9.75M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
      </svg>
    )},
    { label: 'Reward Pool', value: summaryData.rewardsIssued ?? 0, subtext: 'Issued', change: '', trend: 'neutral', icon: (
      <svg className="w-5 h-5" fill="none" stroke="currentColor" strokeWidth="1.8" viewBox="0 0 24 24">
  ...
  const ecosystemStats: any[] = liveData ? [
    { label: 'Active Promotions', value: liveData.liveMetrics?.activePromotions ?? 0, icon: 'business' },
    { label: 'Scans Today', value: liveData.liveMetrics?.scansToday ?? 0, icon: 'leads' },
    { label: 'Redemptions Today', value: liveData.liveMetrics?.redemptionsToday ?? 0, icon: 'gift' },
    { label: 'Activations Today', value: liveData.liveMetrics?.activationsToday ?? 0, icon: 'automation' },
  ] : [];

  if (summaryLoading || liveLoading) {
    return (
      <div className="space-y-8">
        <div>/* header */</div>
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-6 gap-4">
          {Array.from({ length: 6 }).map((_, i) => (
            <div key={i} className="bg-white rounded-2xl border border-[#eee] shadow-sm p-5 animate-pulse">
              <div className="h-8 w-8 bg-[#f0f0f0] rounded-lg mb-3" />
              <div className="h-6 w-16 bg-[#f0f0f0] rounded mb-2" />
        ...
        <div className="lg:col-span-4 bg-white rounded-2xl border border-[#eee] shadow-sm p-6 flex flex-col">
          ...
          <div className="flex-1 space-y-0 overflow-y-auto">
            {activityFeed.length === 0 ? (
              <p className="text-center text-[#888] py-8 text-[13px]">No recent activity</p>
            ) : (
              activityFeed.map((item: any, idx: any) => (
                <div
                  key={item.id}
