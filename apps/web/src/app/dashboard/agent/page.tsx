...
import { useAutomations, useLiveMonitoring } from '@/services/dashboard';
...
export default function AgentTrackerPage() {
  ...
  const steps = useMemo(() => {
    if (!automations || !Array.isArray(automations)) return [];
    return automations.map((item: any, idx: number) => {
      const triggerType = item.triggers?.[0]?.type || 'Event';
      const actionType = item.actions?.[0]?.type || 'Action';
      ...
      return {
        id: idx + 1,
        title: item.name || `${triggerType} ➔ ${actionType}`,
        status,
        time: item.isActive ? 'Active' : 'Paused',
        description: item.description || `Trigger: ${triggerType}, Action: ${actionType}`,
      };
    });
  }, [automations]);
    ...
    return (
      <div className="space-y-10 pb-20">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
          <div className="lg:col-span-8 bg-white rounded-[32px] border border-[#eee] p-8 animate-pulse h-96" />
          <div className="lg:col-span-4 bg-white rounded-[32px] border border-[#eee] p-8 animate-pulse h-96" />
        </div>
      </div>
    );
  }
