      ...
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-5 bg-white border border-[#eee] rounded-3xl p-6 shadow-sm text-left">
        <div className="flex items-center gap-4">
          ...
          <div className="space-y-0.5">
            ...
            <p className="text-[#888] text-[11px]">Rank: Silver Tier • streak score +{profile.totalPoints || profile.points || 0} points</p>
          </div>
        </div>
        ...
        <div className="flex items-center gap-6">
          <div className="text-center">
            ...
            <p className="text-xl font-display font-extrabold text-[#f97316] mt-0.5">{profile.availableSpins ?? profile.dailySpins ?? 0}</p>
          </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
             ...
             <div className="bg-white rounded-3xl border border-[#eee] p-6 shadow-sm space-y-4 text-left">
              ...
              <div>
                ...
                <p className="text-sm text-[#888]">You have {activeRewards} active rewards in your wallet.</p>
              </div>
        ...
        <div className="lg:col-span-4 space-y-6 text-left">
          <div className="bg-white rounded-3xl border border-[#eee] p-6 shadow-sm space-y-6">
            ...
            <div className="flex items-center gap-4">
              <div className="text-4xl font-display font-black text-[#1a1a1a]">{profile.streakDays ?? profile.streak ?? 0}</div>
              ...
              {(activityList.slice?.(0, 5) ?? []).map((act: any) => (
                <div key={act.id} className="flex gap-3">
                  <div className="w-8 h-8 bg-[#fafaf9] rounded-lg border border-[#eee] flex items-center justify-center shrink-0">
                    {(act.type === 'redeem' || act.type === 'redemption') ? (
                  ...
                  <div className="space-y-0.5">
                    <p className="text-[11px] font-bold text-[#1a1a1a] leading-tight">{act.description || act.action}</p>
                    <p className="text-[9px] text-[#bbb] font-semibold">{act.time || act.createdAt}</p>
                  </div>
                </div>
              ))}
            </div>
