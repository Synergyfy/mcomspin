...
export default function FavoritesPage() {
  ...
  const filteredFavorites = favorites.filter((f: any) => 
    (f.name || f.businessName || '').toLowerCase().includes(searchQuery.toLowerCase()) || 
            ...
            filteredFavorites.map((biz: any) => (
              ...
              >
                ...
                <div className="h-40 relative overflow-hidden">
                  <img src={biz.image || biz.bannerUrl || 'https://images.unsplash.com/photo-1441984904996-e0b6ba687e04?q=80&w=2070&auto=format&fit=crop'} alt={biz.name || biz.businessName} className="w-full h-full object-cover group-hover:scale-110 transition-transform duration-700" />
                  <div className="absolute inset-0 bg-gradient-to-t from-[#1a1a1a]/80 via-transparent to-transparent" />
                  
                  {/* Status Badges */}
                  <div className="absolute top-4 left-4 flex gap-2">
                    {biz.hasActiveCampaign || biz.isLive ? (
                ...
                <div className="p-6 space-y-5 flex-1 flex flex-col justify-between">
                  <div className="space-y-1">
                    <div className="flex items-center justify-between">
                      <p className="text-[10px] font-bold text-[#f97316] uppercase tracking-[0.15em]">{biz.category || biz.businessType}</p>
                      <div className="flex items-center gap-1">
                        <Star className="w-3 h-3 text-amber-500 fill-amber-500" />
                        <span className="text-[11px] font-bold text-[#1a1a1a]">{biz.rating ?? biz.averageRating ?? '-'}</span>
                      </div>
                    </div>
                    <h3 className="text-xl font-display font-bold text-[#1a1a1a]">{biz.name || biz.businessName}</h3>
                  </div>
                  ...
                  <div className="grid grid-cols-2 gap-4 py-4 border-y border-[#f5f5f3]">
                    <div className="space-y-0.5">
                      ...
                      <div className="flex items-center gap-1.5">
                        ...
                        <span className="text-[13px] font-bold text-[#1a1a1a]">{biz.rewardsWon ?? biz.totalRewards ?? 0}</span>
                      </div>
                    </div>
                    <div className="space-y-0.5">
                      ...
                      <div className="flex items-center gap-1.5">
                        ...
                        <span className="text-[13px] font-bold text-[#1a1a1a]">{biz.lastVisit ?? biz.lastVisited ?? 'Never'}</span>
                      </div>
                    </div>
                  </div>
                      ...
                      >
                        <div className="flex items-center gap-2">
                          ...
                          <span className="text-[11px] font-bold text-[#1a1a1a] group-hover/link:text-white truncate max-w-[120px]">
                            {biz.activeCampaign || biz.currentCampaign?.name || 'Active Campaign'}
                          </span>
                        </div>
