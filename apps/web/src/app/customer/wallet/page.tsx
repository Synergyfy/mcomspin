...
export default function RewardWalletPage() {
  ...
  const filteredWallet = walletList.filter((item: any) => {
  ...
  const activeCount = walletList.filter((w: any) => w.status === 'active').length;
  const redeemedCount = walletList.filter((w: any) => w.status === 'redeemed').length;
