        ...
        <div className="lg:col-span-4 space-y-6">
          ...
          <div className="bg-white p-6 rounded-2xl border border-stone-100 shadow-sm space-y-4">
            ...
            <div className="space-y-3">
              {[
                { time: '12:45', msg: 'Physics engine re-calibrated' },
                { time: '11:20', msg: 'Gravity shifted to 0.5G' },
                { time: '09:15', msg: 'Prize distribution weighted' }
              ].map((log, i) => (
                <div key={i} className="flex gap-3 text-[10px]">
                  <span className="text-stone-400 font-mono">{log.time}</span>
                  <span className="text-stone-600 font-bold">{log.msg}</span>
                </div>
              ))}
            </div>
          </div>
